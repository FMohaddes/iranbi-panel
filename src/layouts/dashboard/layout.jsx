...
export function DashboardLayout( {
  ...
  return ( <>
      ...
      <LayoutSection
         ...
         * Header
         *************************************** */
        headerSection = { <HeaderBase
          layoutQuery = { layoutQuery }
          disableElevation = { isNavVertical }
          onOpenNav = { mobileNavOpen.onTrue }
          data = { {
            nav : navData , langs : allLangs ,
          } }
          slotsDisplay = { {
            signIn   : false ,
            purchase : false ,
         ...
         * Sidebar
         *************************************** */
        sidebarSection = { isNavHorizontal ? null : (
          <NavVertical
            data = { navData }
            isNavMini = { isNavMini }
            layoutQuery = { layoutQuery }
      ...
      >
        <Main
          isNavHorizontal = { isNavHorizontal } >{ children }</Main >
      </LayoutSection >
    </> );
}
...
function useNavColorVars( theme , settings ) {
      ...
      case "apparent":
        return {
          layout      : {
            ...
            // caption
            "--nav-item-caption-color" : palette?.grey[ 600 ] , // subheader
            "--nav-subheader-color"       : palette?.grey[ 600 ] ,
            "--nav-subheader-hover-color" : palette?.common.white , // item
            "--nav-item-color"             : palette?.grey[ 500 ] ,
            "--nav-item-root-active-color" : palette?.primary.light ,
            "--nav-item-root-open-color"   : palette?.common.white , // bullet
            "--nav-bullet-light-color" : bulletColor.dark , // sub
  ...
  } , [ palette?.background.default , palette?.background.defaultChannel , palette?.common.white , palette?.grey , palette?.primary.light , palette?.text.disabled , palette?.text.primary , palette?.text.secondary , settings?.navColor , settings?.navLayout , ] );
}
